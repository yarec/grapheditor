	...
	if (name != null)
	{
		...
		try
		{
			...
			else
			{
				...
				{
					var save_url = 'filename=' + encodeURIComponent(name) + '&xml=' + encodeURIComponent(xml)
					// console.log(save_url)
					$.ajax({
						url:SAVE_URL  + "?token=" + urlParams['token'] ,
						type: 'POST',
						data: {xml: xml, id: urlParams['id']}, 
					...
					// new mxXmlRequest(SAVE_URL, ).simulate(document, '_blank');
				}
				else
				{
